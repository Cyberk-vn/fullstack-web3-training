...
export class AuthController {
  ...
  @Get('siwe/nonce')
  @HttpCode(HttpStatus.OK)
  @ApiOkResponse({ description: 'Returns a one-time nonce for SIWE (stored server-side).' })
  async getSiweNonce(@Query('address') address?: string) {
    return await this.authService.generateNonce(address)
  }

  @Post('siwe')
  @HttpCode(HttpStatus.OK)
  @ApiOkResponse({ type: TokenResDto, description: 'Verifies SIWE message and returns auth tokens.' })
  async verifySiweSignature(@Body() body: { message: string; signature: string }): Promise<TokenResDto> {
    const { message, signature } = body
    return this.authService.verifySiweSignature(message, signature)
  }
